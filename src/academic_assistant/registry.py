...
SOURCE_FILE = "cwnu.curriculum.2026.changwon-undergraduate.json"
...
class Registry:
    ...
    def load(cls, project_root: Path | str | None = None) -> "Registry":
        try:
            ...
            source = _read_json_once(root / "knowledge" / "sources" / SOURCE_FILE)
    ...
    @staticmethod
    def _validate_profile(rules: list[dict[str, Any]], source: dict[str, Any], research: dict[str, Any], intents: dict[str, Any], pins: dict[str, Any]) -> None:
        expected_ids = {name[:-5] for name in RULE_FILES}
        if len(rules) != 13 or {r.get("rule_id") for r in rules} != expected_ids:
            raise RegistryUnavailable()
        for obj in [source, *rules]:
            if obj.get("review") != {**obj.get("review", {})}:
                raise RegistryUnavailable()
            review = obj["review"]
            if (review.get("status"), review.get("mode"), review.get("scope")) != ("approved", "human", "full"):
                raise RegistryUnavailable()
        if source.get("source_id") != "cwnu.curriculum.2026.changwon-undergraduate":
            raise RegistryUnavailable()
        source_scope = source.get("applicability", {})
        ...
        if any(not alias or any(char.isspace() for char in alias) or alias != alias.lower() for alias in aliases):
            raise RegistryUnavailable()
        if set(pins) != {"schema_version", "algorithm", "rules", "source", "research", "intents"} or pins.get("schema_version") != "1.0.0" or pins.get("algorithm") != "sha256-canonical-json-v1":
            raise RegistryUnavailable()
        computed_rules = {rule["rule_id"]: canonical_sha256(rule) for rule in rules}
        if pins.get("rules") != computed_rules:
            raise RegistryUnavailable()
        if pins.get("source") != {"id": source["source_id"], "digest": canonical_sha256(source)}:
            raise RegistryUnavailable()
